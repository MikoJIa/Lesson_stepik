...
# another solution
def func2(x):
    order = {ind: 'Имя;Зачёт;Оценка;Номер;'.split(';').index(name)
    ...
    t_sorted = tuple(map(tuple, (sorted((int(i) if i.isdigit() else i for i in el.split(';')),
                                 key=lambda x: order[el.split(';').index(str(x))]) for el in x)))
    return t_sorted
...
#         temp = [name, credit, asses, num] if (ind == 0) else [name, credit, int(asses), int(num)]
